...
import com.erp.dtos.request.auth.PasswordResetDTO;
import com.erp.dtos.request.auth.PasswordUpdateDTO;
...
import com.erp.payload.ApiResponse;
import com.erp.services.IAuthService;
...
public class AuthenticationController {
    ...
    @Operation(summary = "Register employee account", description = "Creates a new employee account")
    @PostMapping("/register")
    public ResponseEntity<ApiResponse<UserResponseDTO>> registerEmployee(@Valid @RequestBody RegisterUserDTO registerUserDTO) {
        try {
            logger.debug("Registering employee account for: {}", registerUserDTO.getEmail());
            UserResponseDTO createdUser = userService.createEmployee(registerUserDTO);
            return ApiResponse.success("Employee registered successfully", HttpStatus.CREATED, createdUser);
        } catch (Exception e) {
            logger.error("Employee registration failed for: {}", registerUserDTO.getEmail(), e);
            ExceptionUtils.handleResponseException(e);
            return ApiResponse.fail("Failed to register employee", HttpStatus.BAD_REQUEST, e.getMessage());
        }
    }
